...
def conv(x, channels, kernel=4, stride=2, pad=0, pad_type='zero', use_bias=True, sn=False, scope='conv_0'):
    with tf.variable_scope(scope):
        ...
        if sn:
            w = tf.get_variable("a_kernel", shape=[kernel, kernel, x.get_shape()[-1], channels], initializer=weight_init,
                                regularizer=weight_regularizer)
            x = tf.nn.conv2d(input=x, filter=spectral_norm(w),
                             strides=[1, stride, stride, 1], padding='VALID')
            if use_bias:
                bias = tf.get_variable("a_bias", [channels], initializer=tf.constant_initializer(0.0))
...
def att(x, channels, scope):
    with tf.variable_scope(scope), tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        f = conv(x, channels // 8, kernel=1, stride=1, sn=True, scope='f_conv')
        g = conv(x, channels // 8, kernel=1, stride=1, sn=True, scope='g_conv')
        h = conv(x, channels, kernel=1, stride=1, sn=True, scope='h_conv')
